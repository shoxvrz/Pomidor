...
const OrderPage = () => {
  ...
  const removeHandler = async (id) => {
    try {
      await axios.delete(
        `https://66b482189f9169621ea33d7a.mockapi.io/orders/${id}`
      );
          ...
          return (
            <div key={order.id || i} className="orderPage__main--card">
              <div className="orderPage__main--card--userInfo">
                <b>
                  <span className="media__name">Ism:</span>
                  {order.name}
                </b>
                <b>
                  <span className="media__name">Familiya:</span>
                  {order.surname}
                </b>
                <b>
                  <span className="media__name">Email:</span>
                  {order.email}
                </b>
                <b>
                  <span className="media__name">Tel:</span>
                  {order.telNomer}
                </b>
                <b>
                  <span className="media__name">Davlat:</span>
                  {order.country}
                </b>
                <b>
                  <span className="media__name">Shahar:</span>
                  {order.city}
                </b>
                <b>
                  <span className="media__name">Tuman:</span>
                  {order.tuman}
                </b>
                <b>
                  <span className="media__name">Postkod:</span>
                  {order.postcode}
                </b>
                <b
                ...
                <b>
                  <span className="media__name">Total Price:</span>
                  {totalPrice}.000 so'm
                </b>
              </div>
              <div className="orderPage__main--card--orderList">
                {order.items.map((orderItem, id) => (
                  <div key={orderItem.id || id} className="card">
                    <div>
                      <b>Ovqat nomi:</b>
                      <b>{orderItem.name}</b>
                    </div>
